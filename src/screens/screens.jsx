    ...
    <div>
      ...
      <Container>
        ...
        {/* v39 MILESTONE EXAM — every 3 lessons, a real exam on what you've
            ...
            listening, fill-the-gap). Offered as a celebrated milestone on the
            path, not a hard gate — you can keep learning if you skip it, but
            it's strongly encouraged. This is the "learning path with exams
            every 3 lessons" flow. */}
        {(() => {
          ...
          return (
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1, color: "var(--text)", opacity: 0.7, marginBottom: 4 }}>
                📝 Milestone exam
              </div>
              <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 4 }}>
                {done}-lesson milestone reached!
              </div>
              <div style={{ fontSize: 13, color: "var(--text-dim)", marginBottom: 14, lineHeight: 1.5 }}>
                Time for an exam. {examSize} questions testing your words in
                different ways — recall, recognition, listening, and filling in
                the blank. Pass it and you'll know these have really stuck.
              </div>
              <Button
...
function UnitNode({ unit, index, isCurrent, onTap, onTestOut }) {
  ...
  // Outer container — div so we can have a nested Test-out button on locked
  // cards without nested-button HTML invalidity. The "main" tap is on its own
  // inner button area.
  return (
    <div
      style={{
    ...
    >
      ...
      {/* v27: contextual test-out — for locked units only. Lets a learner who
          already knows the material prove it and skip ahead, right where
          they're staring at the lock, instead of finding it buried in
          Settings. The skip works because TestOut seeds known words globally,
          which lifts the unlock condition once enough are marked known. */}
